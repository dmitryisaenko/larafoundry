...
> **⚠️ Early design note (June 2025).** This page is an early planning draft from before the package was built. The shipped package differs in places: APIs, config keys, and module scope have changed since. Treat this page as design intent, not current reference. For what the package actually ships, see the accurate reference at [../authentication.md](../authentication.md). This file stays at its original URL so older links keep working.
