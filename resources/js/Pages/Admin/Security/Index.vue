<script setup>
/**
 * Operator security (in-console), super-admin only.
 *
 * The operator's own security surface, reached from the AdminLayout pull-out and
 * from the OTP gate's enrolment redirect (`two_factor_setup_route`). Three
 * sections:
 *   1. Two-factor authentication (TOTP) — enrol / confirm / recovery codes /
 *      disable, driving the core's `admin.security.two-factor.*` endpoints (which
 *      wrap Fortify's own actions — see SecurityController).
 *   2. PIN lock — reuses the shared {@see PinManager} section (posts to pin.*,
 *      which the confine already allows).
 *   3. Change password — posts to `admin.security.password.update`.
 *
...
import { AdminLayout, InputField, confirm, useT } from '@dmitryisaenko/larafoundry';
...
<template>
    <AdminLayout :title="$t('Security')">
        <div class="mx-auto flex max-w-2xl flex-col gap-6">
            ...
            <!-- Change password -->
            <section v-if="has_password" class="rounded-md border border-border bg-surface p-5">
                <header class="mb-4">
