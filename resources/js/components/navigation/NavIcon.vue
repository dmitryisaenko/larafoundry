<script setup>
/**
 * Maps a menu item's icon NAME (a string from the backend MenuItem) to an
 * inline SVG from the core's starter set (decision D-nav-f).
 *
 * Icons are inlined here, never published as assets from `vendor/` — that keeps
 * the icon set out of the frontend-under-composer risk (#1). A host extends the
 * set by passing its own name that this component does not know (renders the
 * fallback dot) or by swapping the whole component via its barrel.
 *
 * Unknown / null names render a neutral dot so the menu never breaks on a name
 * the core has not shipped yet.
 */
...
// Heroicons-style 24px stroke paths, keyed by the names the core's providers
// emit. `null` for a name with no glyph yet (renders the fallback).
const PATHS = {
    ...
    broadcast: 'M10.34 15.84c-.688-.06-1.386-.09-2.09-.09H7.5a4.5 4.5 0 1 1 0-9h.75c.704 0 1.402-.03 2.09-.09m0 9.18c.253.962.584 1.892.985 2.783.247.55.06 1.21-.463 1.511l-.657.38c-.551.318-1.26.117-1.527-.461a20.845 20.845 0 0 1-1.44-4.282m3.102.069a18.03 18.03 0 0 1-.59-4.59c0-1.586.205-3.124.59-4.59m0 9.18a23.848 23.848 0 0 1 8.835 2.535M10.34 6.66a23.847 23.847 0 0 0 8.835-2.535m0 0A23.74 23.74 0 0 0 18.795 3m.38 1.125a23.91 23.91 0 0 1 1.014 5.395m-1.014 8.855c-.118.38-.245.754-.38 1.125m.38-1.125a23.91 23.91 0 0 0 1.014-5.395m0-3.46c.495.413.811 1.035.811 1.73 0 .695-.316 1.317-.811 1.73m0-3.46a24.347 24.347 0 0 1 0 3.46',
};

const path = computed(() => (props.name ? PATHS[props.name] ?? null : null));
</script>
...
<template>
    ...
    <span v-else class="inline-block h-1.5 w-1.5 shrink-0 rounded-full bg-current opacity-60" aria-hidden="true" />
</template>
